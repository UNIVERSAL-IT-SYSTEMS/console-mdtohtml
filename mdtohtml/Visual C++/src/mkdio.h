 ...
 */
Document *mkd_in(FILE*,mkd_flag_t);		/* assemble input from a file */
Document *mkd_string(const char*,int,mkd_flag_t);	/* assemble input from a buffer */

/* line builder for github flavoured markdown
 */
Document *gfm_in(FILE*,mkd_flag_t);		/* assemble input from a file */
Document *gfm_string(const char*,int,mkd_flag_t);	/* assemble input from a buffer */
...
__declspec(dllexport) int mkd_generatehtml(Document *p, std::ostringstream &outstream);
...
/* version#.
 */
__declspec(dllexport) char markdown_version[];
